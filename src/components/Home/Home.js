...
class Welcome extends Component {
	...
	initializePanels() {
		return [{name: 'Witness Feed', img: 'fas fa-cogs fa-2x', minSize: 'small', size: 'large', visible: true, id: 0, gridPlacement: {i: '0', x: this.startingX, y: 5, w: 24, h: 31}},
			...
			{name: 'Committee Feed', img: 'fas fa-crown fa-2x', minSize:'small', size: '', visible: false, id: 6, gridPlacement: {i: '5', x: this.startingX, y: 0, w: 24, h: 25}}];
	}

	componentDidUpdate(prevProps) {
		if(prevProps.sideBarOpen !== this.props.sideBarOpen) {
			const stateCopy = Object.assign({}, this.props.gridLayout);
	...
	renderComponent(component) {
		switch(component.id) {
			...
			case 2:
				return <Account id={component.id} calculateComponentHeight={this.calculateComponentHeight.bind(this)} size={component.size} visible={component.visible}/>;
			case 3:
					...
					<Grid className={`${styles['react-grid-layout']} layout`} layout={newLayout} cols={80} compactType={'vertical'} 
						...
						onDragStop={(newLayout, oldItem, newItem, placeholder, e, element)=>this.updateCoordinates(newLayout, oldItem, newItem, placeholder, e, element)}> 
						 <div className={`${styles['react-grid-item']}`} key={'-1'}>
							<SidePanel  calculateComponentHeight={this.calculateComponentHeight.bind(this)} components={components}
...
const mapStateToProps = (state) => ({
	...
	components:state.gridLayout.components,
	gridLayout:state.gridLayout,
	layout:state.gridLayout.layout,
});
